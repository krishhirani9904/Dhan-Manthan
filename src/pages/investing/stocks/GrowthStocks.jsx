...
import { getGrowthStocks, getPriceChange } from '../../../data/stockMarket';
...
function GrowthStocks() {
  ...
  const stocks = getGrowthStocks().sort((a, b) => b.volatility - a.volatility);
  ...
  return (
    <div className={`h-screen flex flex-col ${t.bg.primary} transition-colors duration-300`}>
      <div className={`flex-shrink-0 flex items-center gap-3 px-4 py-3
        ...
        <h1 className={`text-lg font-bold ${t.text.primary}`}>Growth Potential</h1>
      </div>
            ...
            <button key={stock.id}
              ...
              <div className="text-right">
                <p className={`text-sm font-bold ${isUp ? 'text-green-500' : 'text-red-400'}`}>
                  {isUp ? '+' : ''}{(stock.volatility * 100).toFixed(2)}%
                </p>
                <p className={`text-[10px] ${t.text.tertiary}`}>Growth Potential</p>
              </div>
            </button>
          );
        })}
