...
import { YACHTS } from '../../data/itemsData';
    ...
    <div className={`h-screen flex flex-col ${t.bg.primary}`}>
      <div className={`flex-shrink-0 px-4 py-3 ${t.bg.secondary} border-b ${t.border.default}`}>
        <div className="flex items-center gap-3">
          ...
          <h1 className={`text-lg font-bold ${t.text.primary}`}>Yacht Shop</h1>
        </div>
      ...
      <div className="flex-1 min-h-0 overflow-y-auto scrollbar-hide px-3 py-3 space-y-2">
        {YACHTS.map(yacht => (
          <button key={yacht.id} onClick={() => navigate(`/items/yacht-buy/${yacht.id}`)}
            ...
            <div className="p-3">
              <p className={`text-sm font-bold ${t.text.primary}`}>{yacht.name}</p>
              <p className={`text-xs ${t.text.tertiary}`}>Initial cost:</p>
              <p className={`text-sm font-bold ${t.text.brand}`}>{formatCurrency(yacht.price)}</p>
            </div>
          </button>
        ))}
      </div>
